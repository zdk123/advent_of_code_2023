...
impl Hand {
    ...
    fn determine_hand_type(cards: &[CardValue; 5]) -> HandType {
        ...
        let joker_count = card_count.remove(&CardValue::from('J')).unwrap_or(0);
        let max_count = card_count.values().cloned().max().unwrap_or(1);
        ...
        if joker_count == 0 && max_count == 3 && card_count.len() == 2 {
            return HandType::FullHouse;
        }
        // One Joker Count Case
        if joker_count == 1 && card_count.len() == 2 {
            return HandType::FullHouse;
        }
        ...
        // No Joker Case
        if joker_count==0 && max_count==2 && card_count.len()==3 {
            return HandType::TwoPair;
        }

        //One Joker Case
        if joker_count==1 && max_count==2 && card_count.len()==1 {
            return HandType::TwoPair;
        }
